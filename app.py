...
def generate_answer(user_question,retriever):
    ...
    Do NOT answer the question, just reformulate it if needed and otherwise return it as is in Vietnamese.
    ...
    Answer:
    """
    llm_answer = ChatGoogleGenerativeAI(model="gemini-1.5-flash", temperature=0.2,max_tokens=3000)
...
def user_input(user_question, text_chunks,embedding,reranker):
    ...
    chain = generate_answer(user_question,retriever)
...
def main():
    ...
    st.set_page_config("", page_icon=":scroll:")
    if "state" not in st.session_state:
        st.session_state.state = "data_processing"  # Trạng thái ban đầu
    if "messages" not in st.session_state:
        st.session_state.messages = []  # Lịch sử chat
    if "text_chunks" not in st.session_state:
        st.session_state.text_chunks = None

    if st.session_state.state == "data_processing":
        st.title("Lựa chọn dữ liệu")

        cache_path = "text_chunks_cache.pkl"
        option = st.radio(
            " ",("Sử dụng cache", "Tải file mới")
        )
        data_check=False
        if option == "Sử dụng cache":
            with st.spinner("Đang tải dữ liệu từ cache..."):
                st.session_state.text_chunks = load_cache(cache_path)
                if st.session_state.text_chunks is None:
                    st.error("Cache không tồn tại. Vui lòng tải file mới.")
                else:
                    st.success("Dữ liệu đã được tải từ cache.")
                    data_check=True

        elif option == "Tải file mới":
            uploaded_file = st.file_uploader("Tải file PDF lên", type=["pdf"])
            if uploaded_file is not None:
                with st.spinner("Đang xử lý file mới..."):
                    st.session_state.text_chunks = load_chunks(uploaded_file, None)
                    save_cache(st.session_state.text_chunks, cache_path)
                    st.success("Dữ liệu đã được xử lý và lưu vào cache.")
                    data_check=True

        # Nút chuyển sang giao diện hỏi đáp
        if st.button("Bắt đầu hỏi đáp"):
            if data_check:
    ...
    # Input chat
        ...
        back_img="D:\\NLP first\\robot.png"
        with col1:
        ...
        if not st.session_state.messages:
            st.session_state.messages.append({
                "role": "assistant",
                "content": "Chào mừng bạn đến với hệ thống hỏi đáp! Hãy đặt câu hỏi của bạn ở bên dưới."
            })
    ...
    # Input chat
        prompt = st.chat_input("Hãy đặt câu hỏi về dữ liệu của bạn")
        ...
        if prompt:
            ...
            with st.spinner('Đang tìm kiếm câu trả lời...'):
                user_input(prompt, st.session_state.text_chunks, embedding, reranker)
        ...
        with st.sidebar:
            # Nút quay lại giao diện xử lý dữ liệu
            st.title("Tùy chọn")
            if st.button("Quay lại"):
                st.session_state.messages = [] 
                st.session_state.state = "data_processing"
                st.rerun()   
            
            st.write("---") 
            # Tùy chọn đoạn chat mới
